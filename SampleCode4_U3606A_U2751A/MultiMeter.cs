...
namespace CypressSemiconductor.ChinaManufacturingTest
{
    public class MultiMeter
    {
        ...
        public enum DeviceMeasureMode { CURRENT = 1, VOLTAGE = 2}
        ...
        public bool ConfigureU3606A(DeviceMeasureMode measureMode)
        {
            string idnTemp;
            ...
            devReadyStatus = U3606A_Connect(_connectString);
            if (devReadyStatus)
            {
                ioMultiMeterCntrl.WriteString("*RST", true);              // Reset
                ioMultiMeterCntrl.WriteString("*CLS", true);              // Clear
                ...
                switch (measureMode)
                {
                    case DeviceMeasureMode.CURRENT:
                        //ioMultiMeterCntrl.WriteString("CONF:CURR 0.01,0.000001", true); // 10mA range with 1uA resolution 
                        //ioMultiMeterCntrl.WriteString("CONF:CURR 0.01,0.0000001", true);  // 10mA range with 0.1uA resolution
                        ...
                        */
                        ...
                        //ioMultiMeterCntrl.WriteString("CONF:CURR 3,0.0001", true);  // 3A range with 100uA resolution; By my test it is not supported
                        break;
                    case DeviceMeasureMode.VOLTAGE:
                        ioMultiMeterCntrl.WriteString("CONF:VOLT 10,0.001", true);  // This command configures the instrument for DC voltage measurements.The 10 V range is selected with 1 mV resolution.
                        break;
                    default:
                        break;
                }
        ...
        public volt MeasureChannelVoltage(int DelayBeforeMeasureMs, int DelayAfterMeasureMs)
        {
            lock (ioMultiMeterCntrl)
            {
                try
                {
                    ConfigureU3606A(DeviceMeasureMode.VOLTAGE);
                }
                catch (Exception ex)
                {
                ...
                ioMultiMeterCntrl.WriteString("CALC:AVER:AVER?", true);
                ch_voltage.average = (double)ioMultiMeterCntrl.ReadNumber(IEEEASCIIType.ASCIIType_Any, true);

                ioMultiMeterCntrl.WriteString("CALC:AVER:MAX?", true);
                ch_voltage.max = (double)ioMultiMeterCntrl.ReadNumber(IEEEASCIIType.ASCIIType_Any, true);

                ioMultiMeterCntrl.WriteString("CALC:AVER:MIN?", true);
                ch_voltage.min = (double)ioMultiMeterCntrl.ReadNumber(IEEEASCIIType.ASCIIType_Any, true);
